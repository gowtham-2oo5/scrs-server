...
public class StudentController {
    ...
    public ResponseEntity<String> createTest(@RequestBody StudentRegsDTO studentDTO) throws IOException {
        return new ResponseEntity<>(studService.createStudent(studentDTO, null).getName(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<String> deleteStudent(@RequestParam String id) {
        try {
            studService.deleteStudent(UUID.fromString(id));
            return ResponseEntity.ok().body("Student deleted successful");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Error deleting student, e: " + e.getLocalizedMessage());
